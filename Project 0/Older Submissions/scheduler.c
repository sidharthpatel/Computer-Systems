...
/*
 * This is a helper function to insert a process at the front
 * of the Queue by relinking the head pointer to new process
 * followed by incrmenting the number of Processes in the Queue.
 */
void insert_front(Queue *list, Process *p) {
  p -> next = list -> head;
  list -> head = p;
...
/*
 * Remove the process from the the front of the Linked list.
 * Followed by relinking the Linked list and returning removed process.
 * Finally, decrement the number of processes present in that Queue.
 */
 Process *remove_from_front(Queue *list) {
...
int scheduler_reap(Schedule *schedule, int pid) {
  ...
  if(schedule -> defunct_queue -> count == 0) {
	 ...
	 */
  if(schedule -> defunct_queue -> count == 1 || schedule -> defunct_queue -> head -> pid == pid) {
...
/*
 * This is a helper function which searches for the lowest cur_priority Node
 * in the Queue.
 * The function returns a previous node of the Node that needs to be removed.
 * You can see how this concept can cause problems if the header is the lowest
 * cur_priority.
 * As a result, I have used a dummy flag to recognize whether the previous
 * pointer refers to the head being the lowest cur_priority or the head -> next.
 */
Process *find_lowest_priority(Queue *list, int *head_is_select) {
  int head_temp = 0;
...
/*
 * This function is a helper that assists mainly the Scheduler_select function.
 * It takes the given argument of a Process and removes it from the Queue.
 */
Process *delete_func(Queue *list, Process *previous, int *hs){
  Process *temp = list-> head;
...
//  printf("Executing the Scheduler_Count Function\n");
  if(ll == NULL) {
    return -1;
  }
  return (ll -> count);
...
/*
 * This is a helper function that recurses through every process freeing
 * it from its designated memory allocation.
 */
void free_everything(Queue *list){
  Process *temp = list -> head;
...
//  printf("Executing the Scheduler_Free Function\n");
